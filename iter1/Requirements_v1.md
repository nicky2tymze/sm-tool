...
What this means: The tool accepts a path to a PO-Tool-produced iteration-open handoff, validates its shape, and records the iteration as an open entry in `log.jsonl`. This is the entry point of every cycle.

Acceptance: Given a valid PO Tool handoff JSON path, the tool records an iteration-open entry containing the parsed requirements in `log.jsonl` and rejects malformed or duplicate handoffs without partial writes.
...
What this means: At most one iteration is open at a time. Attempting to ingest a new handoff while one is open is a hard error until the active iteration is closed (accepted or force-closed).

Acceptance: Ingesting a second iteration while one is open returns an error and writes no log entry; ingestion succeeds only when no iteration is open.
...
What this means: The tool spawns a one-shot SM Agent from a frozen role spec, feeds it the active iteration's requirements, and records the returned stories as a sequenced backlog in the log. Each story has technical-level acceptance criteria.

Acceptance: After decomposition runs, the log contains a sequenced story backlog tied to the active iteration, where each story has a stable id, ordinal sequence, and technical-level acceptance criteria captured verbatim from the SM Agent output.
...
What this means: The operator specifies an integer cut position N. Stories 1..N become the active sprint; N+1..end are deferred. The cut is recorded as a single log entry referencing story ids on each side.

Acceptance: Given a cut position N within the backlog length, a sprint-cut entry is appended naming the in-sprint and deferred story ids, and subsequent state queries treat only stories 1..N as the active sprint.
...
What this means: Each story moves through a fixed state machine. Every transition is an append-only log entry. Illegal transitions (e.g., planned → accepted) are rejected.

Acceptance: For every in-sprint story, the tool enforces transitions only along planned → in_progress → in_review → accepted | rejected, writes an entry per transition, and rejects any out-of-order transition without writing.
...
What this means: A story cannot reach the `accepted` state without a recorded Reviewer approval that includes a passing test result. Force-close is the only bypass and is logged distinctly.

Acceptance: Transitioning a story to `accepted` requires a logged Reviewer-approved test-pass artifact for that story; absent that artifact, only `rejected` or an explicit force-close path is permitted.
...
What this means: When every in-sprint story is in a terminal state (accepted, rejected, or force-closed), the tool emits a close handoff JSON mapping story outcomes back to PO Tool's requirement ids so PO Tool can mark requirements accepted/rejected upstream. Mirrors PO Tool's close-and-flow shape.

Acceptance: When all in-sprint stories are terminal, the tool writes a close handoff JSON whose shape PO Tool can ingest, records an iteration-close entry in the log, and refuses to close while any in-sprint story is still non-terminal unless force-closed.
...
What this means: The operator can explicitly force-close an iteration when stories cannot be cleanly accepted. Force-close is a distinct, logged action — not a silent acceptance — and still produces a valid close handoff.

Acceptance: A force-close command terminates the active iteration with a distinct log entry, marks remaining non-terminal stories as force-closed in the handoff, and produces a close handoff PO Tool can ingest.
...
1. When PO Tool revises requirements mid-iteration, the customer says "no automatic conflict resolution" — should ingest of a revised handoff for an already-open iteration error out, or should it be a no-op with a warning, or is the rule simply "PO Tool won't do this while an iteration is open"?
2. Is `rejected` a terminal state per story (sprint can close with rejected stories), or must every rejected story be re-worked until accepted before iteration-close?
3. Does the close handoff need to map every story back to a PO Tool requirement id (1:1 or N:1), or does PO Tool only need the requirement-level accept/reject roll-up? This affects what the SM Agent must capture during decomposition.
4. Is force-close gated by a confirmation/reason field, or is the bare command sufficient? (Suite convention from PO Tool's "force-close" path would inform this.)
5. For Iter 1, is the SM Agent spawn synchronous-blocking-on-the-operator-terminal, or fire-and-await like PO Tool's agent calls? Affects whether the tool needs a "resume after agent returns" entry point.
6. Should the sprint-cut command be re-runnable (replace the prior cut) or one-shot per iteration? Re-cut has obvious value if the operator changes their mind before stories start moving.
7. Does the test-pass gate require a structured artifact (e.g., test result JSON the Reviewer references) or is a Reviewer's logged approval entry by itself sufficient evidence?

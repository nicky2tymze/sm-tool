# sm-tool — Iter 2 Requirements v1
...
What this means: The current `NotImplementedError` default in `decompose` is replaced with a function that reads the SM Agent role spec, sends it plus the active iteration's requirements as a single user message to the Anthropic SDK, and returns a parsed story list.

Acceptance: Running `python -m sm decompose` against a real active iteration with `ANTHROPIC_API_KEY` set and no injected `spawn_agent` produces a structured story list and persists it through the existing pipeline contract.
...
What this means: The current `NotImplementedError` default in `execute`'s test-writer surface is replaced with a real Anthropic SDK call that takes the Test Writer role spec and a story dict and returns test source code as a string.
...
What this means: The current `NotImplementedError` default in `execute`'s coder surface is replaced with a real Anthropic SDK call that takes the Coder role spec, the story, and the test code, and returns implementation source as a string.
...
What this means: The current `NotImplementedError` default in `execute`'s reviewer surface is replaced with a real Anthropic SDK call that takes the Reviewer role spec, story, test code, and impl code, and returns a dict with `approved` (bool) and `test_result` (str).

Acceptance: Running `execute <story_id>` end-to-end with no injected `spawn_reviewer` and a valid `ANTHROPIC_API_KEY` produces a correctly-typed `{approved: bool, test_result: str}` dict that drives the existing accept/reject branch logic.
...
What this means: All four real-agent surfaces default to Claude Haiku 4.5; the operator may override via an env var (or equivalent config value) to point a given spawn at a stronger model without code changes.

Acceptance: With no override set, every spawn uses Haiku 4.5; with the override set, spawns use the overridden model — verifiable from request payload or log.
...
What this means: When a real agent call fails (network, auth, malformed response), the failure is surfaced as a typed exception — using `DecomposeAgentError` in the decompose path (this also closes retro item 1) and an appropriate typed error in execute paths — and is not silently swallowed or auto-retried.

Acceptance: A simulated SDK failure during `decompose` raises `DecomposeAgentError`; a simulated SDK failure during `execute` raises a typed error and exits with a documented non-zero exit code.
...
What this means: Either gate `SM_LOG_PATH` behind a `--log-path` CLI flag or rename it to `SM_TEST_LOG_PATH` so production semantics are clear; refresh `_HELP_TEXT` to cover all 12 subcommands and all exit codes 0-11.

Acceptance: An operator reading `--help` sees all 12 subcommands and exit codes 0-11; production code no longer silently reads a test-isolation env var under its old name.
...
What this means: Tighten the structural test to actually grep for inline entry construction across modules, and either implement deep-copy semantics on `build_entry` or rewrite the docstring to honestly describe shallow behavior.

Acceptance: The tightened test fails if a future change reintroduces inline entry construction in any module; the `build_entry` docstring matches actual runtime semantics.
...
1. Override env var name — should the model override be a single `SM_MODEL` applied to all four spawns, or per-spawn (e.g. `SM_DECOMPOSE_MODEL`, `SM_TEST_WRITER_MODEL`, `SM_CODER_MODEL`, `SM_REVIEWER_MODEL`)? Customer Ask says "an env var or config value" but does not specify granularity.
2. Structured-output strategy for `decompose` story list and `reviewer` `{approved, test_result}` — do you want strict JSON-mode / tool-use enforcement, or is a documented "ask for JSON, parse, raise typed error on malformed" acceptable for Iter 2?
3. Role-spec file location — where do the four role-spec files live on disk, and is the path conventional (e.g. `roles/sm.md`, `roles/test_writer.md`) or configurable?
4. For retro item 3 — your preference between (a) `--log-path` CLI flag and (b) renaming env var to `SM_TEST_LOG_PATH`? Both are listed as acceptable; PO recommends the rename for minimum surface change.
5. For retro item 6 — `build_entry` shallow vs deep: PO recommends rewriting the docstring (lower-risk) unless any downstream caller actually depends on deep-copy semantics. Confirm?
6. Token / output-size limits per spawn — any cap to set on `max_tokens` for cost discipline, or leave at SDK default?
7. Exit code for typed agent-failure errors raised from real-agent surfaces — reuse an existing 0-11 code, or allocate a new one (and if new, does the help text refresh in retro item 4 need to cover 12+)?
